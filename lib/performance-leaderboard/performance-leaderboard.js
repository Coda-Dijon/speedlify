...
async function runLighthouse(urls, numberOfRuns = NUMBER_OF_RUNS, options = {}) {
  ...
  // Ensure default Chrome flags are always included, even if options overrides chromeFlags
  if (options && options.chromeFlags) {
    opts.chromeFlags = [...defaultChromeFlags, ...options.chromeFlags];
  } else {
    opts.chromeFlags = defaultChromeFlags;
  }

  log(`Chrome flags: ${opts.chromeFlags.join(' ')}`);
  ...
  for(let j = 0; j < numberOfRuns; j++) {
    ...
    if(!opts.readFromLogDirectory && opts.freshChrome === "run") {
      const launcherConfig = {
        ...
        chromePath: opts.launchOptions.chromePath || chromePuppeteerPath,
        ...opts.launchOptions
      };
      // Ensure chromeFlags aren't overridden by launchOptions
      launcherConfig.chromeFlags = opts.chromeFlags;

      log(`Launching chrome with config: ${JSON.stringify(launcherConfig, null, 2)}`);
